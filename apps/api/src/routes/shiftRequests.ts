...
router.get('/', async (req: Request, res: Response) => {
    ...
    const requests = rows
      .map((row) => {
        ...
        if (!lended) {
          // Data is inconsistent; skip this row
          return null;
        }

        const base = {
          id: String(row.id),
          createdAt: lended.startTime.toISOString(), // no createdAt column; use shift start
          status: statusToString(row.status),
        };

        // TIME OFF: only one shift, requestedShiftId is null
        if (!requested) {
          return {
            ...base,
            ...
            // You can add `reason` later if you add a column
          };
        }

        // TRADE: two shifts, two users
    ...
    console.error(err);
    res.status(500).json({ error: 'Failed to load shift requests' });
  }
});
...
router.get('/:id', async (req: Request, res: Response) => {
    ...
    const base = {
      id: String(row.id),
      createdAt: lended.startTime.toISOString(),
      status: statusToString(row.status),
    };
    ...
    res.status(200).json({ request: requestDto });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error(err);
    res.status(500).json({ error: 'Failed to load shift request' });
  }
});
...
router.post('/:id/approve', async (req: Request, res: Response) => {
    ...
    // Your front-end only checks res.ok, so 204 is fine
    res.status(204).send();
  } catch (err) {
    console.error(err);
